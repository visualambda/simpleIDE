...
void LiteCompleter::setCompletionContext( CompletionContext ctx)
{
    m_completer->setCompletionContext(ctx);
}

 CompletionContext LiteCompleter::completionContext() const
{
...
bool LiteCompleter::startCompleter(const QString &completionPrefix)
{
    ...
    if (this->completionContext() ==  CompleterImportContext && completionPrefix.endsWith("/")) {
        goto end;
    }
...
void LiteCompleter::insertCompletion(QModelIndex index)
{
    ...
    if (m_completer->completionContext() ==  CompleterCodeContext) {
