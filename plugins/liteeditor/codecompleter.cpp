...
int CodeCompleterProxyModel::filter(const QString &filter, int cs,   CompletionContext ctx)
{
    if (!m_model) {
        return 0;
    }
    ...
    foreach (const QChar &c, prefix) {
        if (cs ==   CaseInsensitive ||
                (cs ==   FirstLetterCaseSensitive && !first)) {
...
void CodeCompleterEx::setCompletionContext(  CompletionContext ctx)
{
    m_ctx = ctx;
}

  CompletionContext CodeCompleterEx::completionContext() const
{
    return m_ctx;
}
