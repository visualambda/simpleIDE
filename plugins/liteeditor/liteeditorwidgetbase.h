...
public:
    ...
    LiteEditorWidgetBase(IApplication *app, QWidget *parent = 0);
    ...
    void setEditorMark(IEditorMark *mark);
    void setTextLexer(ITextLexer *lexer);
public:
    QWidget* extraArea();
    ...
    void setNavigateHead(EditorNaviagteType type, const QString &msg);
    void insertNavigateMark(int blockNumber, EditorNaviagteType type, const QString &msg, const QString &tag, int offset = 0, int selection = 0);
    void clearAllNavigateMark(  EditorNaviagteType types, const QString &tag = 0);
...
public slots:
    ...
    void setFindOption(FindOption *opt);
...
private:
    ...
    void updateFindOrSelectionMark(  EditorNaviagteType type);
    bool checkFindOrSelectionMark(  EditorNaviagteType type) const;
    bool checkFindOrSelectionMarkBlock(const QTextBlock &block,
                           EditorNaviagteType type, QTextCursor &cur) const;
protected:
      IApplication *m_liteApp;
    QWidget *m_extraArea;
    QWidget *m_navigateArea;
      IEditorMark *m_editorMark;
    QScopedPointer<  ITextLexer> m_textLexer;
    QMap<  ExtraSelectionKind,QList<QTextEdit::ExtraSelection> > m_extralSelectionMap;
