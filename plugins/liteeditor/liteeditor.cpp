...
//       IEditContext *editContext = new EditContext(this,this);
...
void LiteEditor::setEditorMark(   IEditorMark *mark)
{
    m_editorWidget->setEditorMark(mark);
    m_extension->addObject("LiteApi.IEditorMark",mark);
}

void LiteEditor::setTextLexer(   ITextLexer *lexer)
{
...
void LiteEditor::setCompleter(   ICompleter *complter)
{
    if (m_completer) {
...
{
//       IActionContext *actionContext = m_liteApp->actionManager()->getActionContext(this,"Editor");
...
//    m_editToolBar->setIconSize(   getToolBarIconSize(m_liteApp));
...
void LiteEditor::findNextText()
{
   ...
   IExtension *LiteEditor::extension()
{
    return m_extension;
}
...
//        this->setNavigateHead(   EditorNavigateReload,tr("Reload File"));
//        updateEditorInfo();
//        emit reloaded();
//    }
    return success;
...
int LiteEditor::position(   ITextEditor::PositionOperation posOp, int at) const
{
...
//    if (mgr) {
//           IQuickOpen *p = mgr->findById("quickopen/lines");
//        if (p) {
...
void LiteEditor::setFindOption(   FindOption *opt)
{
...
void LiteEditor::setNavigateHead(   EditorNaviagteType type, const QString &msg)
{
    m_editorWidget->setNavigateHead(type,msg);
}

void LiteEditor::insertNavigateMark(int line,    EditorNaviagteType type, const QString &msg, const QString &tag, int offset, int selection)
{
...
void LiteEditor::clearAllNavigateMark(   EditorNaviagteType types, const QString &tag)
{
    m_editorWidget->clearAllNavigateMark(types, tag);
}

void LiteEditor::showLink(const    Link &link)
{
...
EditContext::EditContext(LiteEditor *editor, QObject *parent)
    :    IEditContext(parent), m_editor(editor)
{
