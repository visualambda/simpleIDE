...
const   EditorNaviagteType MARKTYPE_PRIORITYLIST[PRIORITYLIST_LENGTH] = {
          EditorNavigateError,   EditorNavigateBad,   EditorNavigateWarning,   EditorNavigateReload,   EditorNavigateFind,   EditorNavigateSelection,   EditorNavigateNormal
    };

inline QColor markTypeColor(  EditorNaviagteType type) {
    switch(type) {
    case   EditorNavigateBad:
    case   EditorNavigateError:
        return Qt::darkRed;
    case   EditorNavigateWarning:
        //Returns orange color
        return QColor::fromRgb(255, 125, 0);
    case   EditorNavigateNormal:
        return Qt::darkGreen;
    case   EditorNavigateReload:
        return Qt::darkBlue;
    case   EditorNavigateFind:
        return Qt::darkYellow;
    case   EditorNavigateSelection:
        return Qt::darkBlue;
    }
    return Qt::darkGreen;
}
...
struct NavigateMark
{
    ...
    void addNode(  EditorNaviagteType type, const QString & msg, const QString &tag = "", int offset = 0, int selection = 0)
    {
        Node *node = new Node;
        node->type = type;
        node->msg = msg;
    ...
    Node* findNode(  EditorNaviagteType type)
    {
        foreach (Node *node, m_nodeList) {
            if (node->type == type) {
                return node;
    ...
    void removeNode(  EditorNaviagteType types, const QString &tag = "")
    {
...
public:
    ...
    {
        m_type =   EditorNavigateNormal;
    }
    ~NavigateManager()
    {
        clearAll();
    }
    void insertMark(int blockNumber, const QString &msg,   EditorNaviagteType type, const QString &tag = "", int offset = 0, int selection = 0)
    {
    ...
    void clearAllNavigateMark(  EditorNaviagteType types, const QString &tag = "")
    {
    ...
    void setType(  EditorNaviagteType type, const QString &msg)
    {
        m_type = type;
        m_msg = msg;
    }
...
LiteEditorWidgetBase::LiteEditorWidgetBase(  IApplication *app, QWidget *parent)
...
void LiteEditorWidgetBase::setEditorMark(  IEditorMark *mark)
{
    m_editorMark = mark;
    if (m_editorMark) {
...
void LiteEditorWidgetBase::setTextLexer(  ITextLexer *lexer)
{
...
void LiteEditorWidgetBase::editContentsChanged(int position, int charsRemoved, int charsAdded)
{
    ...
    if (!m_findExpression.isEmpty()) {
        this->updateFindOrSelectionMark(  EditorNavigateFind);
    }
}
...
//        setExtraSelections(  CurrentLineSelection,QList<QTextEdit::ExtraSelection>() << full );
//    }
...
void LiteEditorWidgetBase::navigateAreaPaintEvent(QPaintEvent *e)
{
    ...
    while(i.hasNext()) {
        ...
        if (!i.value()->isEmpty()) {
            ...
            const   EditorNaviagteType * pList = MARKTYPE_PRIORITYLIST;
...
void LiteEditorWidgetBase::slotSelectionChanged()
{
    ...
    if (m_selectionExpression.pattern() != pattern) {
        ...
        updateFindOrSelectionMark(  EditorNavigateSelection);
        viewport()->update();
    }
...
void LiteEditorWidgetBase::setFindOption(  FindOption *opt)
{
    if (!opt) {
    ...
    updateFindOrSelectionMark(  EditorNavigateFind);
...
void LiteEditorWidgetBase::setNavigateHead(  EditorNaviagteType type, const QString &msg)
{
    m_navigateManager->setType(type,msg);
    m_navigateArea->update();
}

void LiteEditorWidgetBase::insertNavigateMark(int blockNumber,   EditorNaviagteType type, const QString &msg, const QString &tag, int offset, int selection)
{
    m_navigateManager->insertMark(blockNumber,msg,type,tag,offset,selection);
    m_navigateArea->update();
}

void LiteEditorWidgetBase::clearAllNavigateMark(  EditorNaviagteType types, const QString &tag)
{
...
void LiteEditorWidgetBase::showLink(const   Link &link)
{
    ...
    setExtraSelections(  LinkSelection,QList<QTextEdit::ExtraSelection>() << sel);
...
void LiteEditorWidgetBase::clearLink()
{
    ...
    setExtraSelections(  LinkSelection, QList<QTextEdit::ExtraSelection>());
    viewport()->setCursor(Qt::IBeamCursor);
    m_currentLink =   Link();
    QToolTip::hideText();
}

bool LiteEditorWidgetBase::openLink(const   Link &link)
{
    ...
    if (!link.targetOpenDir.isEmpty()) {
        ...
        if (mgr) {
              IQuickOpenFolder *folder =   getQuickOpenFolder(mgr);
            if (folder) {
...
void LiteEditorWidgetBase::setExtraSelections(  ExtraSelectionKind kind, const QList<QTextEdit::ExtraSelection> &selections)
{
    m_extralSelectionMap[kind] = selections;
    QList<QTextEdit::ExtraSelection> all;
    QMapIterator<  ExtraSelectionKind,QList<QTextEdit::ExtraSelection> > i(m_extralSelectionMap);
    while(i.hasNext()) {
        i.next();
        all += i.value();
    }
...
void LiteEditorWidgetBase::updateFindOrSelectionMark(  EditorNaviagteType type)
{
...
bool LiteEditorWidgetBase::checkFindOrSelectionMark(  EditorNaviagteType type) const
{
    if (  EditorNavigateFind == type) {
        return !m_findExpression.isEmpty();
    }
    if (  EditorNavigateSelection == type) {
...
bool LiteEditorWidgetBase::checkFindOrSelectionMarkBlock(
    const QTextBlock &block,   EditorNaviagteType type, QTextCursor &cur) const
{
    int pos = 0;
    if (  EditorNavigateFind == type)
