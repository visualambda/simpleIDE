...
QString FileUtil::lookupGoBin(const QString &bin,    IApplication *app, const QProcessEnvironment env, bool bLiteAppPriority)
{
    if (bLiteAppPriority) {
...
QString FileUtil::lookupLiteBin(const QString &bin,    IApplication *app, const QProcessEnvironment env)
{
    QString find = FileUtil::findExecute(app->applicationPath()+"/"+bin);
    if (find.isEmpty()) {
